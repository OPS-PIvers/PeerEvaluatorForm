 ...
 * @param {string} observationName The name for the new observation.
...
function createObservation(observeeEmail, observationName) {
  ...
  try {
    ...
    // This calls the SheetService function with the required 3 arguments
    return createObservation(observeeEmail, observationName, evaluatorEmail);
  } catch (error) {
 ...
 * @return {boolean} True if the note was saved successfully.
 */
function saveNote(observationId, componentId, noteContent) {
  ...
  try {
    // This calls the SheetService function
 ...
 * @return {boolean} True if the rating was saved successfully.
 */
 ...
 * @return {boolean} True if the media was uploaded successfully.
 */
 ...
 * This function is exposed to the client-side.
 * @param {string} observeeEmail The email of the user being observed.
 * @param {string} status Optional status to filter by (e.g., "Finalized").
...
/**
 * Retrieves all details for a single observation.
 * This function is exposed to the client-side.
 * @param {string} observationId The ID of the observation.
...
/**
 * Finalizes an observation, generates the PDF, and locks it.
 * This function is exposed to the client-side.
...
function createObservation(observeeEmail, observationName) {
  ...
  try {
    const evaluatorEmail = Session.getActiveUser().getEmail();
    if (!evaluatorEmail) {
      throw new Error('Could not determine evaluator email.');
    }
    return createObservation(observeeEmail, evaluatorEmail, observationName);
  } catch (error) {
    console.error('Error creating observation:', formatErrorMessage(error, 'createObservation'));
    return null;
  }
...
/**
 * Retrieves all observations for a specific user.
 * This function is exposed to the client-side.
 * @param {string} observeeEmail The email of the user being observed.
 * @return {Array} An array of observation objects.
 */
function getObservations(observeeEmail) {
  try {
    return getObservations(observeeEmail);
  } catch (error) {
    console.error('Error retrieving observations:', formatErrorMessage(error, 'getObservations'));
    return [];
  }
