...
function updateObservationMetadata(observationId, metadata) {
    try {
        ...
        // If the observation name is being updated, rename the corresponding Google Drive folder.
        if (metadata.observationName && metadata.observationName !== observation.observationName) {
            try {
                // Retrieve the folder using the immutable observationId
                const obsFolder = getOrCreateObservationFolder(observationId);
                if (obsFolder) {
                    obsFolder.setName(metadata.observationName);
                    debugLog(`Renamed observation folder for ${observationId} to "${metadata.observationName}"`);
                } else {
                    console.warn(`Could not find observation folder to rename for ID: ${observationId}`);
                }
            } catch (driveError) {
                // Log the error, but don't let a failed folder rename block the metadata update.
